...
export const getDiffInDayHourMinSec = (date1, date2) => {
  ...
  const days = Math.floor(
    diff /
      (DAY_IN_HOUR * HOUR_IN_MINUTE * MINUTE_IN_SECOND * SECOND_IN_MILLISECOND),
  )
  const hours =
    Math.floor(
      diff / (HOUR_IN_MINUTE * MINUTE_IN_SECOND * SECOND_IN_MILLISECOND),
    ) % DAY_IN_HOUR
  const minutes =
    Math.floor(diff / (MINUTE_IN_SECOND * SECOND_IN_MILLISECOND)) %
    HOUR_IN_MINUTE
